...
def user_key_tar_archive_get(org_id, user_id):
    ...
    is_admin = getattr(flask.g, 'administrator', None) and \
               getattr(flask.g.administrator, 'super_user', False)
    ...
    if not is_admin:
        ...
        target_usr = org.get_user(user_id)
        if not target_usr:
            return flask.abort(404)

        if sso_email.lower() not in [
            (target_usr.email or '').lower(),
            (target_usr.name or '').lower(),
        ]:
            logger.warning('Unauthorized profile download attempt', 'key',
                sso_email=sso_email,
                target_user=target_usr.name,
...
def key_request_get():
    ...
    if not doc or doc['type'] not in (KEY_REQUEST_AUTH, SAML_AUTH):
        return flask.abort(404)
    ...
    if not settings.local.sub_active and SAML_AUTH not in sso_mode:
        logger.error('Subscription must be active for sso', 'sso')
        return flask.abort(405)
        logger.error('DEBUG key_request state=%s doc=%s sso=%s' % (str(state), str(doc), str(sso_mode)), 'sso')
