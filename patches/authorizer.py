...
class Authorizer(object):
    ...
    def _check_sso_token(self):
        # Bypass Pritunl enterprise SSO token - no valid license.
        # Real identity verification is done in _check_sso via AWS IDP.
        # Setting has_sso_token=True tells _check_password to skip password.
        if self.has_link:
            return
        logger.info(
            'Skipping Pritunl SSO token, AWS IDP check in _check_sso',
            'authorizer',
            user_name=self.user.name,
        )
    ...
    def _check_sso(self):
        ...
        with open('/tmp/sso_debug.log', 'a') as f:
            f.write('_check_sso called for user: %s auth_type: %s\n' % (
                self.user.name, self.user.auth_type))
   
        if self.user.auth_type == 'local':
            logger.info(
                'Local user bypassing AWS IDP check',
                'authorizer',
                user_name=self.user.name,
            )
            return
        ...
        try:
            ...
            creds.read('/root/.aws/credentials')
            aws_key    = creds['default']['aws_access_key_id']
            aws_secret = creds['default']['aws_secret_access_key']

            IDENTITY_STORE_ID = 'd-90660df585'
            APPLICATION_ARN   = 'arn:aws:sso::285364194150:application/ssoins-722326aeca663a25/apl-722338b2f16ae2d9'
            AWS_REGION        = 'us-east-1'

            user_email = self.user.email or self.user.name
            ...
            for page in paginator.paginate(ApplicationArn=APPLICATION_ARN):
                for assignment in page['ApplicationAssignments']:
                    ...
                    if principal_type == 'USER':
                        ...
                        try:
                            gpaginator = idstore.get_paginator(
            ...
            if allowed:
                ...
                logger.warning(
                    'AWS IDP verification failed - user not in app',
                    'authorizer',
                    user_name=self.user.name,
                    user_email=user_email,
                )
                self.user.audit_event('user_connection',
                    ('User connection to "%s" denied. '
                     'User not authorized in AWS Identity Center') % (
                        self.server.name),
                    remote_addr=self.remote_ip,
                )
                journal.entry(
                    ...
                    self.user.journal_data,
                    self.server.journal_data,
                    event_long='AWS IDP verification failed',
                )
