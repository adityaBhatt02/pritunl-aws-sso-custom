...
def check_token(token, user_id, server_id):
    ...
    if not data or user_id != data['user_id'] or \
        ...
        # Fallback: check MongoDB directly (handles reauth case)
        try:
            tokens_collection = mongo.get_collection('server_sso_tokens')
            doc = tokens_collection.find_one({'_id': token})
            if doc and doc.get('user_id') == user_id and \
                    doc.get('server_id') == server_id:
                with _tokens_lock:
                    _tokens[token] = {
                        'user_id': user_id,
                        'server_id': server_id,
    ...
    if utils.now() - data['timestamp'] > token_ttl:
        return False
    # Refresh timestamp on each successful check so connection stays alive
    with _tokens_lock:
        _tokens[token]['timestamp'] = utils.now()
    return True
def init_token():
